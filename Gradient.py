...
class NeuralNetworkGradientDescent:
    def __init__(self, input_size, hidden_sizes, output_size):
        ...
        self.v_weights = [np.zeros_like(w) for w in self.weights]  # "Скорость" для весов
        self.v_biases = [np.zeros_like(b) for b in self.biases]  # "Скорость" для смещений
    ...
    # Применение градиентного спуска с моментом
    def apply_gradient_descent(self, grads_w, grads_b):
        """Обновление весов с помощью градиентного спуска с моментом"""
    ...
    def load_model(self, filename):
        ...
        # Загрузка состояния оптимизатора, если есть
        if 'v_weights' in model_data:
            self.v_weights = model_data['v_weights']
        if 'v_biases' in model_data:
...
class RockPaperScissorsGame:
    def __init__(self, model_path=None):
        ...
        self.model = NeuralNetworkGradientDescent(input_size, hidden_sizes, output_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="rps_model_gradient.pkl", help="Path to model file")
