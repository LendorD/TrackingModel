...
class NeuralNetwork:
    def __init__(self, input_size, hidden_sizes, output_size):
        ...
        self.reg_lambda = 0.0001  # Коэффициент L2-регуляризации
        self.momentum = 0.9  # Коэффициент момента
    ...
    # Производная ReLU: Нужна для обратного распространения ошибки
    def relu_derivative(self, x):
        return (x > 0).astype(float)

    # Функция активации для выходного слоя (преобразует выходы в вероятности)
    def softmax(self, x):
        # Устойчивый softmax
    ...
    # Вычисляет предсказания сети для входных данных. Сохраняет промежуточные результаты для последующего использования в обратном распространении.
    def forward(self, X):
...
class RockPaperScissorsGame:
    ...
    def train_model(self, data_dir="rps_data", save_path="rps_model.pkl"):
    ...
    def run(self):
        ...
        countdown_duration = 3  # 3 секунды на подготовку
        result_duration = 3  # 3 секунды показа результата
        ...
        while True:
            ...
            if len(bboxes) >= 2:
                ...
                hands = [hands[bboxes.index(bboxes[0])], hands[bboxes.index(bboxes[1])]]
            ...
            if game_state == "ready":
                ...
                if time_left > 0:
                    ...
                    if len(hands) >= 2:
                        for i in range(2):
                            probs = self.model.predict_proba(hands[i].reshape(1, -1))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="rps_model.pkl", help="Path to model file")
